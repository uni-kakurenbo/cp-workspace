...
signed main() {
    ...
    int T, MAX_N, MAX_X; cin >> T >> MAX_N >> MAX_X;

    uniform_int_distribution<int> distr_N(0, MAX_N);
    uniform_int_distribution<int> distr_X(1, MAX_X);

    ofs_in << T << "\n";

    REP(i, T) {
        int n = distr_N(engine), x = distr_X(engine);
        ofs_in << n << " " << x << "\n";
    }
