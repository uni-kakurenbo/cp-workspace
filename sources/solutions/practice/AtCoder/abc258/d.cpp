...
struct PP {
    ll a, b;
    PP() {};
    PP(ll a, ll b) : a(a), b(b) {};
};

signed main() {
    int n, x; cin >> n >> x;
    vector<PP> stages(n), funcs(n);
    REP(i, n) cin >> stages[i].a >> stages[i].b;
    REP(i, n) funcs[i].a = stages[i].b;
    ll sum = 0;
    REP(i, min(n, x)) {
        funcs[i].b = sum + stages[i].a - i * funcs[i].a;
        sum += stages[i].a + stages[i].b;
    }
    ll ans = INF64;
    REP(i, min(n, x)) {
        chmin(ans, funcs[i].a*x + funcs[i].b);
    }
    cout << ans << endl;
    return 0;
}
