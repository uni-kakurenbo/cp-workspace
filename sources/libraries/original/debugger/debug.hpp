...
template<class T, enable_if_t<Internal::is_loggable_v<T>>* = nullptr> string lit(T);

template<class T> string lit(T*);
...
template <class T, enable_if_t<is_arithmetic_v<T>>* = nullptr> string lit(T val) {
    stringstream res;
    res << COLOR_NUMERIC << fixed << setprecision(numeric_limits<T>::digits10);// << scientific;
...
template <size_t N = 0, class T> void iterate_tuple(__attribute__ ((unused)) T val, stringstream &res) {
...
template<class Arg> void DEBUG(nullptr_t, Arg arg) { cdebug << arg << flush; }
template<class ...Args> void DEBUG(Args... args) { cdebug << lit(args...) << flush; }
